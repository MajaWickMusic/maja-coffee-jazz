...
writeProgress({
  ...
  total: items.length,
  percent: 0,
  message: `Preparing ${items.length} Reel upload${items.length === 1 ? "" : "s"}...`
});
...
for (let index = 0; index < items.length; index += 1) {
  ...
  writeProgress({
    ...
    message: `Uploading ${index + 1}/${items.length}: ${item.title || basename(item.video || "Reel")}`
  });
  ...
  if (!item.video || !existsSync(item.video)) {
    ...
    writeProgress({
      ...
      message: `Skipped ${index + 1}/${items.length}: missing video`
    });
    continue;
  }
  ...
  writeProgress({
    ...
    total: items.length,
    percent: Math.round(((index + 1) / items.length) * 100),
    message: `${dryRun ? "Prepared" : "Uploaded"} ${index + 1}/${items.length}: ${item.title || basename(item.video)}`
  });
}
...
writeProgress({
  ...
  total: items.length,
  percent: 100,
  message: "Upload complete. Videos are ready for scheduled publishing."
});
